...
#define SLEEP_SEC 1
#define SLEEP_1 0 // SLEEP * SLEEP_1 + SLEEP
#define SLEEP_2 0 // SLEEP * SLEEP_2 + SLEEP
...
int main(void)
{
    ...
    char timefrm[BUF_DATETIME_SZ], link[BUF_LINK_SZ],
    ...
    for (;; sleep(SLEEP_SEC))
    {
        ...
        if (count_1 > SLEEP_1)
        {
            ...
            fgets(link, BUF_LINK_SZ, fp);
            fclose(fp);
            trimnewln(BUF_LINK_SZ, link);
        }
        ...
        snprintf(status, BUF_SZ, "%s:%s%%  %s:%s  %s",
                 BATTERY, capacity, IFACE, link, timefrm);
